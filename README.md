...
- **Finance** — net liquid (linked live to the budget, including allowances and rent collected), an editable savings / e-wallet / portfolio, **read-only Solana wallet tracking** (paste a public address — never a key — and its live SOL balance + price fold into your net worth), realized P&L with a last-10-sessions chart, and a trade journal you can log into. Hit **+ EDIT** to inline-edit balances and add/remove holdings.
...
**Macros are wired for a real LLM.** The meal logger is a MyFitnessPal-style food search: as you type it merges three sources — a built-in table of ~120 everyday foods (instant, works offline, includes Filipino dishes like kare-kare, sinigang and adobo), live **Open Food Facts** product results, and an **AI search** ([`app/api/nutrition/search`](app/api/nutrition/search/route.ts)) that returns macros for *any* food (any dish, any cuisine, any brand) via Claude. Picking a result logs it with its macros. Logging free-text also calls `estimateFoodSmart` → [`app/api/nutrition/estimate`](app/api/nutrition/estimate/route.ts). Both routes call Claude (via the official `@anthropic-ai/sdk`) when `ANTHROPIC_API_KEY` is set, and otherwise return `501` so the client falls back to the built-in table + Open Food Facts — the app stays fully functional offline. The `{ kcal, p, c, f }` contract is all the UI knows, so swapping in a nutrition provider (USDA FoodData Central, Nutritionix, etc.) is a one-file change.
